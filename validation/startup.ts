...
export const startupSchema = yup.object().shape({
    ...
    reach: yup.number().required('Patients reached is required').positive('Must be positive').integer('Must be a whole number'),
    region: yup.number().required('Regions covered is required').positive('Must be positive').integer('Must be a whole number'),
